...
class sub_index_code:

    """Calculate the sub_index_score for each stringency category. 
    Inputs: 
        policy_value (integer) = ordinal scale measurement that represents level of policy
                                 stringency. 
        flag (integer) =  dummy variable indicating whether or not an indicator uses a flag 
                          to distinguish between regionally and federally employed policies. 
        recorded_flag (integer) = dummy variable indicating whether the policy is implemented regionally 
                         or federally.
